...
function createNameIndexes(name) {
    ...
    // Create lowercase version for exact matching
    const name_lc = name.toLowerCase().trim();

    // Create first character for prefix searching
    const name_lc_first1 = name_lc.length > 0 ? name_lc.charAt(0) : "";

    return { name_lc, name_lc_first1 };
}
